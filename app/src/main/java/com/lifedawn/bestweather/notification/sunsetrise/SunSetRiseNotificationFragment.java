package com.lifedawn.bestweather.notification.sunsetrise;
	...
	public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
		...
		binding.sunRiseSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (!init) {
					...
					sharedPreferences.edit().putBoolean(getString(R.string.pref_key_sun_rise_notification), isChecked).apply();
				} else {
					init = false;
				}
			}
		...
		binding.sunSetSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (!init) {
					SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(getContext());
					sharedPreferences.edit().putBoolean(getString(R.string.pref_key_sun_set_notification), isChecked).apply();
				} else {
					init = false;
				}
			}
