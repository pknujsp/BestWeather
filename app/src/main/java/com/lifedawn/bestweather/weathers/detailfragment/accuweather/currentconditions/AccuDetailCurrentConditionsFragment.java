...
public class AccuDetailCurrentConditionsFragment extends BaseDetailCurrentConditionsFragment {
	...
	public void setValuesToViews() {
		...
		addGridItem(R.string.temperature, item.getTemperature().getValue(), 0);
		addGridItem(R.string.real_feel_temperature, item.getRealFeelTemperature().getValue(), 0);
		addGridItem(R.string.humidity, item.getRelativeHumidity(), 0);
		addGridItem(R.string.dew_point, item.getDewPoint().getValue(), 0);
		addGridItem(R.string.wind_direction, item.getWind().getDirection().getDegrees(), 0);
		addGridItem(R.string.wind_speed, item.getWind().getSpeed().getMetric().getValue(), 0);
		addGridItem(R.string.wind_gust, item.getWindGust().getSpeed().getMetric().getValue(), 0);
		...
		addGridItem(R.string.pressure, item.getPressure().getMetric().getValue(), 0);
		addGridItem(R.string.uv_index, item.getuVIndexText(), 0);
		addGridItem(R.string.visibility, item.getVisibility().getMetric().getValue(), 0);
		addGridItem(R.string.cloud_cover, item.getCloudCover(), 0);
		addGridItem(R.string.precipitation_volume, item.getPrecip1hr().getMetric().getValue(), 0);
		addGridItem(R.string.precipitation_type, item.getPrecipitationType(), 0);
	}
}
