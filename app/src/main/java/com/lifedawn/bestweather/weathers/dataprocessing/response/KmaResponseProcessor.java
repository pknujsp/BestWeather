...
public class KmaResponseProcessor extends WeatherResponseProcessor {
	...
	public static List<FinalHourlyForecast> getFinalHourlyForecastList(UltraSrtFcstRoot ultraSrtFcstRoot, VilageFcstRoot vilageFcstRoot) {
		...
		ZonedDateTime now =
				ZonedDateTime.now(ZoneId.of(TimeZone.getTimeZone("Asia/Seoul").getID())).withMinute(0).withSecond(0).withNano(0);
		...
		for (Map.Entry<Long, List<VilageFcstItem>> entry : hourDataListMap.entrySet()) {
			...
			now = now.withYear(Integer.parseInt(fcstDate.substring(0, 4))).withMonth(Integer.parseInt(fcstDate.substring(4, 6)))
					.withDayOfMonth(Integer.parseInt(fcstDate.substring(6, 8))).withHour(Integer.parseInt(hourlyFcstItems.get(0).getFcstTime().substring(0, 2).substring(0, 2)));
	...
	public static List<FinalDailyForecast> getFinalDailyForecastList(MidLandFcstRoot midLandFcstRoot, MidTaRoot midTaRoot, Long tmFc) {
		...
		finalDailyForecastList.add(
				new FinalDailyForecast(now.toLocalDateTime(), midLandFcstData.getWf8(), midLandFcstData.getRnSt8(), midTaFcstData.getTaMin8(),
						midTaFcstData.getTaMax8()));

		now = now.plusDays(1);

		finalDailyForecastList.add(
				new FinalDailyForecast(now.toLocalDateTime(), midLandFcstData.getWf9(), midLandFcstData.getRnSt9(), midTaFcstData.getTaMin9(),
