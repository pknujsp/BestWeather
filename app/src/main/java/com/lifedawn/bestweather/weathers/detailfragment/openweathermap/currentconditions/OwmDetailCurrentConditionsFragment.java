...
public class OwmDetailCurrentConditionsFragment extends BaseDetailCurrentConditionsFragment {
	...
	public void setValuesToViews() {
		...
		addGridItem(R.string.dew_point, ValueUnits.convertTemperature(current.getDewPoint(), tempUnit).toString(), R.drawable.dewpoint,
				null);
		addGridItem(R.string.wind_direction, current.getWind_deg(), R.drawable.winddirection, null);
		addGridItem(R.string.wind_speed, ValueUnits.convertWindSpeed(current.getWind_speed(), windUnit).toString(), R.drawable.windspeed,
				null);
		addGridItem(R.string.wind_gust,
				current.getWindGust() == null ? notData : ValueUnits.convertWindSpeed(current.getWindGust(), windUnit).toString(),
		...
		addGridItem(R.string.pressure, current.getPressure(), R.drawable.pressure, null);
		addGridItem(R.string.uv_index, current.getUvi(), R.drawable.uv, null);
		addGridItem(R.string.visibility, ValueUnits.convertVisibility(current.getVisibility(), visibilityUnit), R.drawable.visibility,
				null);
		addGridItem(R.string.cloud_cover, current.getClouds(), R.drawable.cloudiness, null);
		addGridItem(R.string.rain_volume, current.getRain() == null ? "0" : current.getRain().getPrecipitation1Hour(),
				R.drawable.rainvolume, null);
		addGridItem(R.string.snow_volume, current.getSnow() == null ? "0" : current.getSnow().getPrecipitation1Hour(),
