...
public class OwmDetailCurrentConditionsFragment extends BaseDetailCurrentConditionsFragment {
	...
	public void setValuesToViews() {
		...
		addGridItem(R.string.temperature, ValueUnits.convertTemperature(current.getTemp(), tempUnit).toString(), 0);
		addGridItem(R.string.real_feel_temperature, ValueUnits.convertTemperature(current.getFeelsLike(), tempUnit).toString(), 0);
		...
		addGridItem(R.string.wind_speed, ValueUnits.convertWindSpeed(current.getWind_speed(), windUnit).toString(), 0);
		...
		addGridItem(R.string.visibility, ValueUnits.convertVisibility(current.getVisibility(), visibilityUnit).toString(), 0);
		addGridItem(R.string.cloud_cover, current.getClouds(), 0);
		addGridItem(R.string.rain_volume, current.getRain() == null ? "0" : current.getRain().getPrecipitation1Hour(), 0);
		addGridItem(R.string.snow_volume, current.getSnow() == null ? "0" : current.getSnow().getPrecipitation1Hour(), 0);
	}
}
